...
public class IndexFileLoader {
    ...
    public void init(){
        ...
        adCreativeString.forEach(
                adCreate->AdLevelDataHandler.handleLevel2(
                        JSON.parseObject(
                                adCreate, AdCreativeTbale.class
                        ),
                        OpType.ADD
                )
        );
        ...
        adCreativeUnitString.forEach(
                adCreativeUnit->AdLevelDataHandler.handlevel3(
                        JSON.parseObject(
                                adCreativeUnit, AdCreativeUnitTbale.class
                        ),
                        OpType.ADD
                )
        );
        ...
        adUnitDistrictString.forEach(
                adUnitDistrict->AdLevelDataHandler.hanfleLevel4(
                        JSON.parseObject(
                                adUnitDistrict,
                                AdUnitDistrictTable.class),
                        OpType.ADD
                )
        );
